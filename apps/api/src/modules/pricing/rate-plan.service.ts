...
export class RatePlanService {
  ...
  async list(): Promise<RatePlanView[]> {
   ...
   */
  async update(edit: RatePlanEdit): Promise<RatePlanView> {
    const changes = {
    ...
    // no business taking a row lock.
    if (Object.keys(changes).length === 0) {
      return await this.read(edit.code);
    }
  ...
  private async read(code: RatePlanCode): Promise<RatePlanView> {
