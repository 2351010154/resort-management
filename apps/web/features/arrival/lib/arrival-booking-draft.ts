...
//
// Three places on this page ask for the same stay: the compact rail under the
// bar, the room deck's `Book your stay`, and chapter 7's card. They are not
// three questions — a reader who types a range into the rail and then presses
// the deck's booking link has already answered, and asking again would be the page
// forgetting. So the draft lives here and every one of them reads and writes it.
//
 ...
 * above `MAX_CHILD_AGE`. Checked here so the guest is told on the card rather
 * than having the funnel silently drop half of what they typed.
 */
 ...
 *
 * For the persistent `Book your stay` in the bar, the one on each room card and
 * the `Choose dates` links in the table and the rituals — all of them addresses
 * rather than submits: they cannot refuse to navigate, so they
 * never validate. An incomplete range is simply not written, and the guest lands
 * on the calendar with their party intact — which is the difference between
 * this and a bare `/booking` that throws the party away.
 */
export function draftHref(draft: ArrivalBookingDraft): string {
  const checkIn = parseDay(draft.from);
  const checkOut = parseDay(draft.to);
  const ordered = checkIn && checkOut && checkIn.compare(checkOut) < 0;
